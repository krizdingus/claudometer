...
void ScreenSession::build(lv_obj_t *parent) {
  ...
  lv_obj_set_style_text_color(resets_label_, theme::fg_muted(), 0);
  ...
  lv_obj_set_style_arc_color(arc_, theme::bar_bg(), LV_PART_MAIN);
  lv_obj_set_style_arc_color(arc_, theme::accent(), LV_PART_INDICATOR);
  lv_obj_clear_flag(arc_, LV_OBJ_FLAG_CLICKABLE);

  // Center: % used (hero number)
  pct_label_ = lv_label_create(arc_);
  lv_obj_set_style_text_color(pct_label_, theme::fg(), 0);
  ...
  lv_obj_set_style_text_color(model_a_, theme::fg_muted(), 0);
  ...
  lv_obj_set_style_text_color(model_b_, theme::fg(), 0);
